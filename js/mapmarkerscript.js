...
daum.maps.event.addListener(map, 'click', function(mouseEvent) {
  searchDetailAddrFromCoords(mouseEvent.latLng, function(result, status) {
    if (status === daum.maps.services.Status.OK) {
      ...
      var detailAddr = '위도 : ' + mouseEvent.latLng.getLat() + '\u00A0\u00A0\u00A0' + '경도 : ' + mouseEvent.latLng.getLng();
      var content = '<div class="bAddr">' + detailAddr + '</div>';
      marker.setPosition(mouseEvent.latLng);
      marker.setMap(map);
      infowindow.setContent(content);
  ...
  for (i = 0; i != files.length; ++i) {
    ...
    reader.onload = function(e) {
      ...
      workbook.SheetNames.forEach(function(item, index, array) {
        ...
        for (var j = 1; range.e.r >= j; j++) {
          juso[j] = (worksheet["A"+j] ? worksheet["A"+j].v : undefined);
          markname[j] = (worksheet["B"+j] ? worksheet["B"+j].v : undefined);
          category[j] = (worksheet["C"+j] ? worksheet["C"+j].v : undefined);
        }
          ...
          geocoder.addressSearch(addr, function(result, status) {
            if (status === daum.maps.services.Status.OK) {
              ...
              if (coords[index] != null) {
                var tempContent = '<button type="button" class = "customMarkButton" id="tempId" onclick="closeOverlay(this.id)">' + markname[index] + '</button>';
                //커스텀오버레이 생성
              ...
              //주소 검색 불가 시 데이터 저장
              jusoNotFound[notFoundCount++] = juso[index];
...
function alertNotFound() {
  ...
  document.getElementById("jusoNotFoundContent").innerHTML = alertNotFoundString;
  goDetail()
}
